...
### 16.2 ¿Cómo usa PX4 parámetros como `CA_ROTOR0_*`?

Cuando usas **Control Allocation (CA)**, PX4 no “adivina” tu dron: construye una matriz geométrica
con los parámetros de cada rotor y, a partir de ella, reparte empuje/par para producir:

- empuje total (subir/bajar),
- momento de roll,
- momento de pitch,
- momento de yaw.

Ejemplo (uno de los rotores hover):
...
- **`CA_ROTOR0_PX` y `CA_ROTOR0_PY`**  
  Definen la posición del rotor respecto al centro de masa (brazo de palanca).  
  Ese brazo determina cuánta autoridad aporta en **roll/pitch**.

- **`CA_ROTOR0_AZ`**  
  Define la dirección del eje de empuje del rotor.  
  En multirrotor clásico tipo “upward thrust”, suele representarse con signo negativo en FRD/NED
  según convención de PX4 (`-1` en muchos airframes SITL).

- **`CA_ROTOR0_CT`**  
  Coeficiente de empuje relativo.  
  Si aumentas este valor, ese rotor “pesa más” en la asignación de thrust.

- **`CA_ROTOR0_KM`**  
  Coeficiente de momento de reacción (torque del rotor), clave para **yaw**.  
  El signo (`+/-`) diferencia sentido de giro CW/CCW para que PX4 cancele o genere yaw correctamente.

Resultado: PX4 combina estos términos de **todos** los rotores (`CA_ROTOR1_*`, `CA_ROTOR2_*`, etc.)
y resuelve cuánto mandar a cada actuador (`HIL_ACTUATOR_CONTROLS.controls[i]`).

> Si estos parámetros no reflejan tu geometría real, el síntoma típico es:
> arma, giran motores, pero hay poca o nula estabilización.
